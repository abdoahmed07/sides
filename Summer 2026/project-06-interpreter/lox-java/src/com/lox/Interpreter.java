...
// Tree-walking interpreter — visits each AST node and directly executes it
// No compilation step: walk the tree, evaluate, done.
// The Resolver pass pre-computes variable scope depths so we don't need to search at runtime.

public class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {

    // Global scope — where built-in functions live
    final Environment globals = new Environment();
    private Environment environment = globals;

    // Maps each variable expression to how many scopes up it lives (set by Resolver)
    private final Map<Expr, Integer> locals = new HashMap<>();

    public Interpreter() {
        // Built-in: clock() — useful for benchmarking
    ...
    public Void visitReturnStmt(Stmt.Return stmt) {
        ...
        throw new Return(value); // unwind the stack with an exception
    }

    @Override
    public Void visitClassStmt(Stmt.Class stmt) {
        // Resolve superclass first so it exists before we create the class
        ...
        // Create a scope for "super" if there's a superclass
        if (stmt.superclass != null) {
    ...
    public Object visitLogicalExpr(Expr.Logical expr) {
        Object left = evaluate(expr.left);
        // Short-circuit evaluation — only evaluate right if needed
    ...
    private String stringify(Object object) {
        ...
        if (object instanceof Double) {
            ...
            // Print integers without the trailing .0
